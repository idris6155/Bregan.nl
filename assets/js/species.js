(() => {
  ...
  const bagFor=p=>{const id=(p.id||'').toLowerCase();if(/bretox|brefung|salbreg/.test(id))return 'assets/images/bag-squares.svg';if(/bre-fe-amino|bre-zn-amino/.test(id))return 'assets/images/bag-yellow.svg';if(/bremax|fishmix/.test(id))return 'assets/images/bag-feather.svg';return 'assets/images/bag-wave.svg'};
  const productCards=products.slice(0,8).map(p=>'<a class="species-product reveal" href="product.html?id='+encodeURIComponent(p.id)+'&lang='+lang+'"><div class="species-product-visual"><img class="species-bag-art" src="'+bagFor(p)+'" alt="Bregan packaging"><span class="species-product-name">'+esc(p.name)+'</span></div><div class="species-product-copy"><small>'+esc(p.type)+'</small><h3>'+esc(p.name)+'</h3><p>'+esc(p.summary?.[lang]||p.summary?.en||'')+'</p><b>'+c('View product','Produkt ansehen')+' ↗</b></div></a>').join('');
  host.innerHTML=
    '<section class="species-hero '+config.hero+'"><div class="species-hero-art"><span>'+config.symbol+'</span></div><div class="container species-hero-grid"><div class="species-hero-copy reveal in-view"><div class="eyebrow">BREGAN · '+config.kicker+'</div><h1>'+esc(config.title[lang==='de'?1:0])+'</h1><p>'+esc(config.intro[lang==='de'?1:0])+'</p><div class="hero-actions"><a class="btn btn-orange" href="products.html?species='+encodeURIComponent(species)+'&lang='+lang+'">'+c('Explore products','Produkte entdecken')+'</a><button class="btn btn-ghost" data-open-navigator>'+c('Find a solution','Lösung finden')+'</button></div><div class="species-hero-meta"><div><strong>'+products.length+'</strong><span>'+c('mapped products','zugeordnete Produkte')+'</span></div><div><strong>'+config.challenges.length+'</strong><span>'+c('challenge routes','Problemrouten')+'</span></div></div></div></div></section>'+
