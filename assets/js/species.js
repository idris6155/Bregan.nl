(() => {
  ...
  const productCards=products.slice(0,8).map(p=>'<a class="species-product reveal" href="product.html?id='+encodeURIComponent(p.id)+'&lang='+lang+'"><div class="species-product-visual '+toneFor(p)+'"><img class="species-bag-art" src="'+(p.image||bagFor(p))+'" alt="'+esc(p.name)+' packaging"></div><div class="species-product-copy"><small>'+esc(p.type)+'</small><h3>'+esc(p.name)+'</h3><p>'+esc(p.summary?.[lang]||p.summary?.en||'')+'</p><b>'+c('View product','Produkt ansehen')+' ↗</b></div></a>').join('');
  host.innerHTML=
